...
impl LspManager {
    ...
    pub async fn start_server(
        ...
        let stdin = child
            .stdin
            .take()
            .ok_or("Failed to open stdin for LSP process")?;
        let stdout = child
            .stdout
            .take()
            .ok_or("Failed to open stdout for LSP process")?;

        let pending_requests =
            Arc::new(TokioMutex::new(HashMap::<i64, oneshot::Sender<Value>>::new()));
                        ...
                        if let Ok(val) = serde_json::from_slice::<Value>(&buf) {
                            ...
                            } else if let Some(method) = val.get("method").and_then(|v| v.as_str())
                            {
                                ...
                                let payload = serde_json::json!({
                                    ...
                                    "params": LspNotificationPayload {
                                        server_id: server_id_clone.clone(),
                                        method: method.to_string(),
                                        params,
                                    }
                                });
                                let _ = notif_tx.send(payload).await;
                            }
                        }
                    }
                }
        ...
        let _ = self
            .send_request(server_id.clone(), "initialize".into(), init_params)
            .await;
        let _ = self
            .send_notification(server_id.clone(), "initialized".into(), serde_json::json!({}))
            .await;
    ...
    pub async fn stop_server(&self, server_id: String) -> Result<(), String> {
        ...
        if let Some(server) = guard.remove(&server_id) {
            let _ = self
                .send_request(server_id.clone(), "shutdown".into(), serde_json::json!({}))
                .await;
            let _ = self
                .send_notification(server_id, "exit".into(), serde_json::json!({}))
                .await;
